...
import { SquaresFour, Package, Barcode, MapPin, SlidersHorizontal, MagnifyingGlass, ArrowUpRight, ArrowRight, ArrowClockwise, CaretRight, CaretLeft, CaretDown, DownloadSimple, Printer, Check, X, Plus, Clock, ListChecks, WarningCircle, Sun, Moon, Command, Scan, FileText, Tray, ChartBar, SignOut, ArrowUp, ArrowDown, CircleNotch } from '@phosphor-icons/react'
...
import { money, signed, normalize, matches, labels, exportProducts, dateLabel, type Filter } from './data'
...
export default function Workspace({ products, locations, membership, email, loading, lastSynced, onRefresh, onEdit, scan, settings, toast }: Props) {
  ...
  const jump = (p: Page, f: Filter = 'all', loc = 'all') => { setPage(p); setFilter(f); setLocationFilter(loc); setQuery(''); window.scrollTo({ top: 0 }) }
...
function Catalogue({ products, filter, setFilter, query, setQuery, locationFilter, setLocationFilter, onEdit }: { products: ProductOverview[]; filter: Filter; setFilter: (f: Filter) => void; query: string; setQuery: (q: string) => void; locationFilter: string; setLocationFilter: (s: string) => void; onEdit: (p: ProductOverview) => void }) {
  ...
  const filtered = useMemo(() => products.filter(p => matches(p, filter) && (locationFilter === 'all' || p.location_code === locationFilter) && normalize(`${p.internal_code} ${p.description} ${p.barcode ?? ''}`).includes(normalize(query.trim()))), [products, filter, query, locationFilter])
  useEffect(() => { setSelection({}) }, [filter, query, locationFilter])
  const columns = useMemo<LegacyColumnDef<ProductOverview>[]>(() => [
    { id: 'select', enableSorting: false, header: ({ table }) => <input aria-label="Επιλογή τρέχουσας σελίδας" type="checkbox" checked={table.getIsAllPageRowsSelected()} onChange={table.getToggleAllPageRowsSelectedHandler()} />, cell: ({ row }) => <input aria-label={`Επιλογή ${row.original.internal_code}`} type="checkbox" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} /> },
...
function CommandSearch({ open, setOpen, products, onPick }: { open: boolean; setOpen: (b: boolean) => void; products: ProductOverview[]; onPick: (p: ProductOverview) => void }) {
  ...
  const found = products.filter(p => normalize(`${p.internal_code} ${p.description} ${p.barcode ?? ''}`).includes(normalize(q.trim()))).slice(0,9)
