...
export type PhotoJob=StoredPhoto&{id:string;owner:string;store:string;created:number;updated:number;state:PhotoState;input:CaptureInput|null;message:string;result:any;retryAt:number}
...
export async function enqueuePhoto(blob:Blob,owner:string,store:string){
 ...
 await photoDb.photos.add({id,owner,store,created:now,updated:now,state:'queued',...await encodePhoto(blob),input:null,message:'Αποθηκεύτηκε στη συσκευή',result:null,retryAt:0});changed();return id
}
...
export function processPhotos(owner:string,store:string):Promise<number>{
 ...
 activeScope=owner+':'+store;let worked=false
 draining=(async()=>{
  let committed=0
  ...
  for(const job of jobs){
   ...
   worked=true;clearTimeout(idle)
   const session=(await supabase.auth.getSession()).data.session;if(session?.user.id!==owner)break
   try{
    let input=job.input
    if(!input){
     const source=restorePhoto(job);if(!source)throw new Error('Η αρχική φωτογραφία δεν είναι διαθέσιμη.')
     ...
     await update(job.id,{...await encodePhoto(image.blob),input,state:'sending',message:'Έτοιμο για Jev'})
    }
    ...
    const current=await photoDb.photos.get(job.id);const evidence=current?restorePhoto(current):null;if(!evidence)throw new Error('Λείπει η φωτογραφία τεκμηρίωσης.')
    await update(job.id,{state:'sending',message:'Ο Jev αντιστοιχίζει τα πεδία'})
    const upload=await supabase.storage.from('capture-evidence').upload(input.evidencePath,evidence,{contentType:'image/jpeg',upsert:false})
    ...
    if(data.state==='committed'||data.state==='duplicate'){
     await update(job.id,{state:data.state,result:data,blob:null,bytes:null,message:data.state==='committed'?'Καταχωρίστηκε αυτόματα':'Η ίδια φωτογραφία έχει ήδη υποβληθεί',retryAt:0});if(data.state==='committed')committed++
  ...
  return committed
 })().finally(()=>{draining=null;if(worked)idle=setTimeout(stopWorkers,120000)})
 return draining
}
