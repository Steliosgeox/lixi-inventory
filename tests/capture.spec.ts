...
async function openFallback(page:Page,name:'Smart Scan'|'Μεμονωμένο OCR'|'Σελίδα Α4'){
  const details=page.locator('.capture-fallbacks')
...
test.beforeEach(async({page})=>{
  ...
  await openFallback(page,'Smart Scan')
...
test('legacy offline price draft remains available as a fallback workflow',async({page,context})=>{
  await openFallback(page,'Μεμονωμένο OCR')
  ...
  await openFallback(page,'Σελίδα Α4')
  await expect(page.locator('.capture-draft')).toHaveCount(1)
...
test('Smart Scan, fallback OCR and A4 modes remain accessible on mobile',async({page})=>{
  for(const name of ['Smart Scan','Μεμονωμένο OCR','Σελίδα Α4'] as const){
    await openFallback(page,name)
