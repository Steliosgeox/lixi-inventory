...
type Item = Product & {new_product:boolean}
...
export type Bundle = {input:CaptureInput;items:Record<string,Item>;prices:Record<string,Candidate<number>>;dates:Record<string,Candidate<{iso:string;kind:string}>>;lots:Record<string,Candidate<string>>;questions:Record<string,Question>;state:Record<string,unknown>;blocking:string[];decoded:string[]}
...
export function buildBundle(input:CaptureInput,products:Product[],structured:StructuredBarcode[]=[]):Bundle {
 ...
 if(!matching.length&&codes.length===1&&decoded.length===1){
  ...
  if(unit&&unitLines.every(l=>(l.score??0)>=.97))for(const [i,l] of descriptions.slice(0,3).entries())items['new_'+i]={id:'',internal_code:codes[0],barcode:decoded[0],description:l.text.trim(),unit,updated_at:'',catalog_price:null,new_product:true}
 }
 ...
 return {input,items,prices,dates,lots,decoded,blocking,questions,state:{ocr:lines,decoded_gtins:decoded,structured_barcodes:structured,product_candidates:items,captured_at:input.observedAt},}
}
...
export function decide(bundle:Bundle,raw:unknown):Decision {
 ...
 if(!p)reasons.push('product_not_identified');if(!price&&!date)reasons.push('no_captured_price_or_expiry')
